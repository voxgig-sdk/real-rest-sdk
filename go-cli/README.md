...
boru-driven command-line client **and** interactive REPL for the RealRest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/real-rest-cli)
make build
...
./real-rest-cli load 1 object            # {id:1} shorthand
./real-rest-cli load '{id:1}' object       # explicit match map
./real-rest-cli update '{name:"x"}' object

# 5. Override the API base URL for a single call
REAL_REST_BASE=https://api.example.com ./real-rest-cli list object

# 6. No arguments -> interactive REPL
./real-rest-cli
real-rest> list object
real-rest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/real-rest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/real-rest-cli list object
   ```
...
```sh
./real-rest-cli load 1 object          # scalar shorthand for {id:1}
./real-rest-cli load '{id:1}' object     # explicit match map
```
...
```sh
./real-rest-cli update '{id:1,name:"new"}' object
```
...
Run with no arguments to open a REPL (prompt `real-rest>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `object`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/real-rest-cli`. |
...
(`./real-rest-cli <expr>`) and interactively (the REPL), and expressions compose the
