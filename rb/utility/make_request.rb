...
module RealRestUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = RealRestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RealRestResponse.new(fetched)
    else
