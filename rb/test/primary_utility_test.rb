...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = RealRestOmni.make_runner(TEST_JSON_FILE, RealRestSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : RealRestOmni.errify(err)
    end
