...
The Lua SDK for the RealRest API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Object(nil):list(nil, nil)
...
```lua
local result, err = client:Object(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
```lua
-- Create
local created, _ = client:Object(nil):create({ name = "Example" }, nil)
...
local result, err = client:RealRest(nil):load(
  { id = "test01" }, nil
)
