...
local RealRestSDK = {}
RealRestSDK.__index = RealRestSDK
...
RealRestSDK._make_feature = _make_feature


function RealRestSDK.new(options)
  local self = setmetatable({}, RealRestSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function RealRestSDK:options_map()
...
function RealRestSDK:get_utility()
  return Utility.copy(self._utility)
end


function RealRestSDK:get_root_ctx()
  return self._rootctx
end


function RealRestSDK:prepare(fetchargs)
...
function RealRestSDK:direct(fetchargs)
...
-- by passing it.
function RealRestSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function RealRestSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "RealRestSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function RealRestSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = RealRestSDK.new(sdkopts)
  sdk.mode = "test"
