# RealRest Python SDK Reference

Complete API reference for the RealRest Python SDK.
...
#### `RealRestSDK.test(testopts=None, sdkopts=None)`
...
```python
result, err = client.Object().create({
    "name": # `$STRING`,
})
```
...
```python
results, err = client.Object().list({})
```
...
```python
result, err = client.Object().remove({"id": "object_id"})
```
...
```python
result, err = client.Object().update({
    "id": "object_id",
    # Fields to update
})
```
