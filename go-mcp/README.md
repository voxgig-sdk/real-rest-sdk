...
[MCP](https://modelcontextprotocol.io) server exposing the RealRest SDK as
two agent tools — `real-rest_list` and `real-rest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/real-rest-mcp)
make build

# 2. Provide credentials via the environment
export REAL_REST_APIKEY=sk_live_xxx
...
```jsonc
// real-rest_list: first page of records
{ "entity": "object" }
{ "entity": "object", "query": { } }

// real-rest_load: one record by id
{ "entity": "object", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/real-rest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user real-rest \
     -- "$PWD"/dist/*/real-rest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `real-rest_list` and `real-rest_load` tools now appear
   in new sessions. Ask the agent to *"list object using real-rest"*
   and it calls `real-rest_list` with `{"entity":"object"}`.
...
```sh
export REAL_REST_APIKEY=sk_live_xxx            # API key
export REAL_REST_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `real-rest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `real-rest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REAL_REST_APIKEY` | API key sent with every request. |
| `REAL_REST_BASE` | Optional override of the API base URL. |
