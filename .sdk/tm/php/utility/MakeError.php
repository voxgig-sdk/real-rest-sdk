...
class RealRestMakeError
{
    public static function call(?RealRestContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RealRestContext([], null);
        }
        $op = $ctx->op ?? new RealRestOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RealRestResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RealRestError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new RealRestError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof RealRestError) {
