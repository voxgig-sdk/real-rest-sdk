# RealRest TypeScript SDK Reference

Complete API reference for the RealRest TypeScript SDK.
...
```ts
new RealRestSDK(options?: object)
```
...
#### `RealRestSDK.test(testopts?, sdkopts?)`
...
**Returns:** `RealRestSDK` instance in test mode.
...
```ts
const results = await client.Object().list({ id: "example_id" })
```
...
Return the parent `RealRestSDK` instance.
