...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.REALREST_TEST_LIVE ||
    'TRUE' === process.env.REALREST_TEST_OVERRIDE
  ) {
  ...
  m.REALREST_TEST_EXPLAIN = process.env.REALREST_TEST_EXPLAIN || m.REALREST_TEST_EXPLAIN
