...
//     (\`new RealRestSDK(...)\` / \`RealRestSDK.test(...)\` become
//     \`RealRestSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"object":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RealRestSDK(...)\` and any
// \`RealRestSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RealRestSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RealRestSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
