...
import (
	...
	omni "github.com/voxgig-sdk/real-rest-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RealRestError); is {
				out := map[string]any{
					"name":    "RealRestError",
...
func omniCtx(arg any, client *sdk.RealRestSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RealRestSDK); is {
			client = live
