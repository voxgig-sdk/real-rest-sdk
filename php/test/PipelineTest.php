...
{
    private static function utility(): RealRestUtility
    {
        return new RealRestUtility();
    }
    ...
    private static function ctx(array $over = []): RealRestContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RealRestContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RealRestOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RealRestError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RealRestOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RealRestMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RealRestOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RealRestMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RealRestOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RealRestMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RealRestSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RealRestMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RealRestResponse([]),
            'result' => new RealRestResult([]),
        ]);
        [, $err] = RealRestMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RealRestSpec([]),
            'response' => null,
            'result' => new RealRestResult([]),
        ]);
        [, $err] = RealRestMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RealRestSpec(['step' => 's']),
            'response' => new RealRestResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RealRestResult([]),
        ]);
        [, $err] = RealRestMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RealRestSpec(['step' => 's']),
            'response' => new RealRestResponse(self::resp(200, ['v' => 1])),
            'result' => new RealRestResult([]),
        ]);
        [, $err] = RealRestMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RealRestSpec(['step' => 's']),
            'response' => new RealRestResponse(self::resp(200, ['v' => 2])),
            'result' => new RealRestResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RealRestMakeResponse::call($ctx);
    ...
    {
        $preset = new RealRestResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RealRestSpec([]),
            'response' => new RealRestResponse([]),
            'result' => new RealRestResult([]),
        ...
        [$got, $err] = RealRestMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RealRestResult([])]);
        [, $err] = RealRestMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RealRestSpec([]), 'result' => null]);
        [, $err] = RealRestMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RealRestSpec(['step' => 's']),
            'result' => new RealRestResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RealRestOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RealRestMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RealRestSpec(['step' => 's']),
            'result' => new RealRestResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RealRestOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RealRestMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RealRestResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RealRestSpec([]),
            'result' => new RealRestResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RealRestMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RealRestMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RealRestError('boom', 'boom');
        $utility->fetcher = function (RealRestContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RealRestSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RealRestMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RealRestContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RealRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RealRestResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RealRestContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RealRestSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RealRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RealRestResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RealRestSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RealRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RealRestMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RealRestMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RealRestResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RealRestDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RealRestResult(['ok' => false])]);
        $this->expectException(RealRestError::class);
        RealRestDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RealRestResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RealRestMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RealRestMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RealRestBaseFeature();
        $b = new RealRestBaseFeature();
        RealRestFeatureAdd::call($ctx, $a);
        RealRestFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RealRestBaseFeature
    {
        $f = new RealRestBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RealRestFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RealRestContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RealRestSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RealRestPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RealRestResponse(['headers' => null]),
            'result' => new RealRestResult([]),
        ]);
        RealRestResultHeaders::call($ctx);
