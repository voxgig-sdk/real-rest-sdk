...
class FtCtrl extends RealRestControl
{
...
class FtClient
{
    ...
    private RealRestUtility $utility;

    public function __construct(RealRestUtility $utility, array $options)
    {
    ...
    public function get_utility(): RealRestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RealRestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RealRestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RealRestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RealRestConfig::make_feature($name);
    ...
    private static function buildUrl(RealRestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RealRestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RealRestResponse($fetched);
        }
    ...
    private function fail(RealRestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RealRestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
